...
<t:snippet title="${book.title} ${book.author}">
    <jsp:body>
        <div style="margin-top: 60px; margin-left: 60px; width: 60%">
            ...
            <h4><b><fmt:message key="book.details.price.field"/> : ${book.price} <fmt:message key="book.details.currency"/></b></h4>
            ...
            <c:if test="${role == OPERATOR}">
                <button type="button" class="btn btn-outline-primary">
                    <a style="text-decoration: none" href="${path}/do/?action=show-edit-book-page"><fmt:message key="book.details.operator.button.message"/></a>
                </button>
            </c:if>
        </div>
    </jsp:body>
