...
public class BookService extends AbstractService {
    ...
    public List<Book> getAllBooks() throws ServiceException {
        return getBooks(GET_BOOKS_ALL);
    }

    public List<Book> getBooksByDomain() throws ServiceException {
        return getBooks(GET_BOOKS_BY_DOMAIN);
    }

    private List<Book> getBooks(String getBooksByDomain) throws ServiceException {
        List<Book> bookList = null;
        try {
            DaoFactory daoFactory = DaoFactory.createJdbcDaoFactory();
            BookDao bookDao = daoFactory.getBookDao();
            bookList = bookDao.getAllByParam(getBooksByDomain);
